...
var ecstatic = module.exports = function (dir, options) {
  ...
  return function middleware (req, res, next) {
    ...
    if (streamMap) {
      if (path.basename(file) == streamMap.file) {
        file = path.normalize(path.join(root, streamMap.file))
      } else if (streamMap.template === true) {
        file = path.normalize(path.join(root, streamMap.file))
      }
    }
    ...
    function serve(stat) {
      ...
      if ((streamMap) && (streamMap.file == path.relative(opts.root, file))) {
        var x = stream.pipe(streamMap.stream);
        x.pipe(res)
        x.on('end', function() {
          res.statusCode = 200
          res.end()
